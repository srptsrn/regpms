class JobVice < ActiveRecord::Base
  ...
  has_many :job_vice_results, -> {order("created_at").where(["workflow_state IN (?)", [:enabled, :confirmed]])}
  
  has_many :job_vice_files, -> {order("created_at")}
  accepts_nested_attributes_for :job_vice_files, :reject_if => :all_blank, :allow_destroy => true
  
  has_many :job_vice_logs, -> {order("created_at")}
  ...
  def submit
    job_vice_files.each do |jvf| 
      jvf.workflow_state = :enabled
      jvf.save
    end
  end
