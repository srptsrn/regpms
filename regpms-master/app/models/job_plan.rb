class JobPlan < ActiveRecord::Base
  ...
  has_many :job_plan_results, -> {order("created_at").where(["workflow_state IN (?)", [:enabled, :confirmed]])}
  
  has_many :job_plan_files, -> {order("created_at")}
  accepts_nested_attributes_for :job_plan_files, :reject_if => :all_blank, :allow_destroy => true
  
  has_many :job_plan_logs, -> {order("created_at")}
