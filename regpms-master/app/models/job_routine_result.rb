class JobRoutineResult < ActiveRecord::Base
  ...
  has_many :job_routine_result_files, -> {order("created_at").where(["workflow_state IN (?)", [:enabled, :confirmed]])}
  accepts_nested_attributes_for :job_routine_result_files, :reject_if => :all_blank, :allow_destroy => true
  
  validates :user_id, presence: true
  validates :evaluation_id, presence: true
  validates :job_routine_id, presence: true
  ...
  def submit
    job_routine_result_files.each do |jrf| 
