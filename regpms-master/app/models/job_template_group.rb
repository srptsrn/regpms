class JobTemplateGroup < ActiveRecord::Base
  ...
  after_initialize do
    if self.new_record?
      self.sorting ||= (JobTemplateGroup.last_sorting ? JobTemplateGroup.last_sorting.sorting.to_i + 1 : 1)
    end
  end
  ...
  def self.last_sorting
    JobTemplateGroup.limit(1).order("sorting DESC").first
  end
