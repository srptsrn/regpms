...
class StrategyGroup < ActiveRecord::Base
  ...
  scope :all_enabled, -> { order(:no, :name).where(workflow_state: :enabled) }
  
  belongs_to :workflow_state_updater, class_name: "User"
  
  has_many :strategies, -> { order("sorting").where(workflow_state: :enabled) }
  accepts_nested_attributes_for :strategies, :reject_if => :all_blank, :allow_destroy => true
